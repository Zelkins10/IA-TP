...
public class AlgoGen : MonoBehaviour
{
    ...
    public Terrain current_terrain;
    ...
    void Update()
    {
        // TODO : compatibility erronée quand ils changent de terrain
        current_terrain = this_controller.corresponding_terrain;
        EnvironmentParameters environment_parameters =
            current_terrain.GetComponent<EnvironmentParameters>();
        compatibility = 0.0f;
        compatibility +=
            Mathf.Abs(environment_parameters.temperature - temperature) / (50.0f - (-20.0f)); // TODO : need to be automatised
        compatibility += Mathf.Abs(environment_parameters.altitude - altitude) / (3000.0f - 0.0f); // need to be automatised
        compatibility += Mathf.Abs(environment_parameters.humidity - humidity) / (100.0f - 0.0f); // need to be automatised
        compatibility /= 3.0f;
    }
    ...
    public void search_best_correspondance()
    {
        ...
        int current_id = AddBoids.list_of_boids.IndexOf(gameObject);
        ...
        current_couple.Add(AddBoids.list_of_boids[max_id]);
        current_couple.Add(AddBoids.list_of_boids[second_id]);
        current_couple.Add(AddBoids.list_of_boids[min_id]);
